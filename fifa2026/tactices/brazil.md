...
- Role mapping (roster order in `brazil.yaml`):
  ...
  - index 1: LB — **Carlos Augusto** — modern overlapping/inverting fullback, comfortable tucking into midfield as a third pivot.
  ...
  - index 4: RB — **Vanderson** — high-and-wide overlapping fullback; provides the width on the right because Raphinha drifts inside.
  ...
  - index 8: AM/10 — **Lucas Paquetá** — the connector, the 10 between the lines, half-space dribbler, the team's best progressive passer in tight spaces.
  ...
  - index 10: ST — **Rodrygo** — false-nine / support striker, drops between the lines, links play, makes diagonal runs in behind from a withdrawn position.

## Style of Play
### Build-up
**Patient short build-up.** Alisson plays out from the back as a rule. Marquinhos and Gabriel split wide; Bruno or Casemiro drops between them when needed. Fullbacks (especially Carlos Augusto) often invert to form a 3-2 build-up shape. The deepest playmaker is **Bruno Guimarães** — the team is built to get him on the ball facing forward. Brazil will pass 5-7 times in their own half to draw the press before going long.

### Pressing
**Mid-block default, with selective high-press triggers.** Press triggers: opposition GK passing to a CB with a teammate within 5 yards (jump the angle), CB receiving with back to play. Vinícius will press the opposition RB; Raphinha presses the LB. **Rodrygo curves his run** to press the deeper CB. Casemiro is the man behind, intercepting any vertical pass through the middle.

### Defensive shape
Out-of-possession: **4-4-2 mid-block** with Paquetá pushing up alongside Rodrygo. Vinícius drops to LM (a key Ancelotti demand), Raphinha drops to RM. Compact between the lines; Brazil concedes the wide channels to deny the central penetration.

### Wide play
**Asymmetric:** Vinícius wide-and-high on the left, Carlos Augusto inverts. Raphinha drifts inside on the right, Vanderson overlaps high. This creates a lopsided shape: wide-left, narrow-right.

### Final third
Patterns: **Vinícius isolation** in the left half-space — get him 1v1 vs the RB, no help, let him cook. **Raphinha-Vanderson combinations** down the right — give-and-go around the fullback. **Paquetá between the lines** finding Rodrygo's diagonal run in behind. **Cutbacks** from the byline to the edge of the box for Bruno's arriving shot. Set-piece delivery from Raphinha is a major weapon.
...
- Penalties: **Vinícius** primary, **Raphinha** secondary, **Rodrygo** tertiary.
...
4. **If my `player_id` ends with `_4` (RB, Vanderson) and team_phase == "attacking" and ball is on the right or in central third:** Sprint to the byline — overlap automatic.
5. **If my `player_id` ends with `_1` (LB, Carlos Augusto) and team_phase == "attacking":** Invert into the half-space alongside `_6` (Casemiro) — form 3-2 build-up. Do NOT overlap LW `_9` (Vinícius) unless he checks back inside.
6. **If my `role == "MID"` and the carrier's `player_id` is not `_5` (Bruno Guimarães):** Move to give `_5` a passing option in space.
7. **If my `player_id` ends with `_10` (ST/false-9, Rodrygo) and team_phase == "attacking":** Drop into the AM space when `_8` (Paquetá) pushes higher; make a diagonal run in behind when `_5` (Bruno) receives facing forward.
...
## Key Player Notes
- **Vinícius (10):** No defensive tracking duty beyond LM line. Free to isolate vs RB. Always 1v1 the first defender.
...
- **Paquetá (7):** The 10. Half-space dribbler. Trusted to take risks in the final third.
