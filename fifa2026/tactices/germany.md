...
- Role mapping (roster order in `germany.yaml`):
  - index 0: GK — Marc-André ter Stegen (sweeper-keeper; pass 17; primary build-up node)
  - index 1: LB — David Raum (stays wide and high; left-flank crosser)
  - index 2: LCB — Antonio Rüdiger (aggressive, physical, ball-carrier into midfield)
  - index 3: RCB — Jonathan Tah (more conservative, covers Kimmich's vacated zone)
  - index 4: RB — Joshua Kimmich (inverts to RCM; pass 18, skill 18 — the brain when he steps inside)
...
### Build-up
- ter Stegen central. Rüdiger and Tah split wide.
- In possession: Kimmich steps inside from RB to form a 3-2 base (Rüdiger-Tah-Raum back, Kimmich-Pavlović pivot) → 3-2-5 with Wirtz, Musiala, Sané, Havertz and Goretzka in 5-line.
...
- High line; Tah and Rüdiger step up aggressively. Risky against pace.
...
## Set Pieces
- Corners: Kimmich is the primary taker. Inswingers from the right (left foot from the right corner). Targets: Rüdiger, Tah, Havertz.
- Direct FKs: Kimmich centrally and right-side; Raum on the left.
- Defending: zonal back-post wall, man-mark on Rüdiger's nearest threat; ter Stegen commands.
...
4. When my `player_id` ends with `_2` (LCB — Rüdiger) and pressure is low: carry the ball forward into midfield; pass to the `_7` (Wirtz) between lines.
...
- **ter Stegen (idx 0)** — sweeper-keeper; treat him as an extra outfield passer.
