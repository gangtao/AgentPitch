...
- Role mapping (roster index -> tactical role):
  ...
  - 5 Saibari — Left #8, half-space progressor with late box arrivals.
  - 6 El Aynaoui — Single pivot #6, ball-winner, screener, and deep distributor.
  - 7 Ounahi — Right #8, box-to-box carrier.
  - 8 Rahimi — Left winger, direct runner who attacks the space in behind with a striker's finishing.
...
### Build-up
- Bounou splits the center-backs; El Aynaoui drops in to form a back-three when pressed.
...
- El Kaabi presses the ball-side CB; nearest winger jumps the full-back; Saibari/Ounahi step onto the pivot.
...
- El Aynaoui shields the back four; the 8s screen the half-spaces.
...
- Left side: Rahimi holds width and attacks the space in behind, Mazraoui underlaps or inverts depending on the ball location.
...
- Late midfield runs from Ounahi and Saibari into the box.
...
- Defensive set pieces: zonal with El Aynaoui picking up the late runner.
...
2. If role == "DEF" and player_id ends with "_2" (Aguerd, LCB) and pressed by one striker: dribble-step forward; if pressed by two, pass to the pivot (player_id ends with "_6", El Aynaoui) dropping in.
3. If player_id ends with "_6" (El Aynaoui, #24 pivot): never shoot from outside 25m; prioritize simple lateral passes that switch play to the RB (player_id ends with "_4", Hakimi).
...
6. If player_id ends with "_8" (Rahimi, LW #9): hold width and attack the space in behind when the LB (player_id ends with "_1", Mazraoui) underlaps; tuck inside as a second striker when Mazraoui overlaps.
...
10. If defending in own third: nearest #8 (player_id ends with "_5" Saibari or "_7" Ounahi) tracks the runner, the pivot (player_id ends with "_6", El Aynaoui) covers the central screen.
...
- **El Kaabi** is an old-school nine and proven poacher — feed him crosses, especially near-post deliveries. His shoot rating (17) is the team's highest.
- **El Aynaoui** rarely carries past the halfway line; his job is to win duels, recycle, and hit the deep switch (pass 16).
