...
- Role mapping (roster order in `norway.yaml`):
  ...
  - index 5 (`norway_5`, Thorstvedt): CM left — box-to-box, late runs into the box, defensive cover.
...
Mid-block, situational press. Haaland triggers when an opposition CB receives with back to play; the wingers curve their runs to lock out the wide CB. Otherwise the front three drop into a 4-5-1 to protect the central spine. Thorstvedt and Berge are responsible for the press's second wave.
...
7. If my player_id ends with "_5" (CM, Thorstvedt, #18): when Norway loses the ball in opposition half, immediately TACKLE the nearest opponent (gegenpress trigger from CM only).
...
10. If my player_id ends with "_1" (LB, Wolfe): overlap aggressively when "_8" (Nusa) cuts in; only one of ("_1" Wolfe, "_5" Thorstvedt) goes forward at a time.
...
## Tournament Mindset
Norway are dangerous in any moment but vulnerable to teams that exploit central midfield gaps when Thorstvedt pushes forward. They will trade chances; in a high-scoring shootout they back themselves to outscore anyone because of Haaland — and now carry a second aerial threat in Sørloth.
