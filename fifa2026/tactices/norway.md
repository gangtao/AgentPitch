...
- Role mapping (roster order in `norway.yaml`):
  ...
  - index 7 (`norway_7`, Aursnes): CM left — box-to-box, supports Ødegaard, defensive cover.
...
6. If my player_id ends with "_5" (DM, Berge, #8): never venture beyond the halfway line in open play unless team is trailing — stay as the defensive anchor.
7. If my player_id ends with "_7" (CM, Aursnes, #14): when Norway loses the ball in opposition half, immediately TACKLE the nearest opponent (gegenpress trigger from CM only).
...
10. If my player_id ends with "_1" (LB, Wolfe): overlap aggressively when "_8" (Nusa) cuts in; only one of ("_1" Wolfe, "_7" Aursnes) goes forward at a time.
...
- **Fredrik Aursnes (index 7):** versatile box-to-box engine; shuttles to cover Ødegaard's forward bursts and leads the gegenpress from central midfield.
