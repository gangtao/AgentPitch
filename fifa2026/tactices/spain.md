...
- Role mapping (roster order in `spain.yaml`):
  ...
  - index 5: LCM/#8 — Dani Olmo (left-side advanced interior; arrives in box late, half-space passer & finisher)
...
- Both FBs step into midfield height; Pedri and Olmo float between lines.
...
- Oyarzabal leads, Yamal & Williams curve runs to lock the FBs. Pedri/Olmo jump #6.
...
- Cutback target: Oyarzabal at the penalty spot, Olmo arriving late from deep.
- Yamal's shot from the right half-space onto his left foot is a recurring termination.

## Set Pieces
- Corners: Inswingers from Yamal on right, Olmo on left. Oyarzabal + Laporte + Cubarsí attack near/back posts. Rodri at edge of box for rebounds.
- Direct FKs: Yamal (right side), Olmo (left), Laporte (central, power).
- Penalties: Oyarzabal is first taker (cool finisher — winner in the Euro 2024 final), then Yamal.
...
5. When my `player_id` ends with `_5` or `_7` (#8 pair — Olmo/Pedri) and team has the ball: position in opposite half-space from the ball; when received, turn forward — never backward — pass 19.
...
11. Shoot from outside the box only if my `player_id` ends with `_10`, `_5`, or `_2` (Yamal/Olmo/Laporte) AND there's a clear lane.
...
- **Olmo (idx 5)** — advanced left-side interior who arrives in the box; doubles as Spain's left-side dead-ball deliverer and a genuine goal threat from the half-space.
