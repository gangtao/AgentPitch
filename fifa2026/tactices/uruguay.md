...
- Role mapping (roster order in `uruguay.yaml`):
  - index 0: GK — **Sergio Rochet** — first-choice keeper, modest sweeper-keeper duties, primarily a shot-stopper, plays short when possible (veteran Muslera is the backup, in a record 5th World Cup).
...
### Wide play
**Asymmetric:** on the **RIGHT**, Pellistri stays wide and attacks the touchline 1v1, with Varela overlapping. On the **LEFT**, de Arrascaeta drifts inside into the half-space, vacating the touchline for the high-and-wide Olivera to overlap. Crosses (especially low cutbacks) for Núñez and Valverde's late runs.

### Final third
Patterns: **Núñez channel runs** — Bentancur or de Arrascaeta plays vertical into the run, Núñez sprints behind. **Valverde late arrival** at the edge of the box — when the ball goes wide, Valverde sprints into the D for a shot. **de Arrascaeta pockets** — receives between the lines and slides through-balls or shoots. **Pellistri isolation** — give him the ball wide and ask him to beat his man. **Set pieces** delivered hard and flat to attack the near post.
...
12. **If my `role == "GK"` (player_id `_0`, Rochet) and pressed:** Play short to a CB if safe; if not, punt long to `_9` (Núñez)'s channel run — never play it long without a target.
