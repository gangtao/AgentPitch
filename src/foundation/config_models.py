...
class SimulationConfig(BaseModel):
    ...
    # Laws 13/15/16/17: a restart taker (free kick, throw-in, corner,
    # goal kick) must put the ball in play with Pass or Shoot (Move is
    # blocked while the restart is pending). If the taker's strategy stalls
    # this many ticks (default 20 = 2 s at tick_rate 10), the engine kicks
    # for them — a pass to the nearest teammate — so an unaware strategy
    # never deadlocks into the stalemate turnover.
    restart_auto_kick_ticks: int = Field(default=20, ge=1, le=300)
